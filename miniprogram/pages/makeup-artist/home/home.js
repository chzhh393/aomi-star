...
Page({
  ...
  loadData() {
    ...
    const makeupArtistId = userInfo.id;

    // 获取所有候选人
    const allCandidates = getAllCandidates();

    // 1. 筛选待服务的候选人
    // 条件：status = 'interview_scheduled' 且 interviewSchedule.interviewers 包含当前化妆师
    const upcomingCandidates = allCandidates.filter(candidate => {
      ...
      return candidate.interviewSchedule.interviewers.some(
        interviewer => interviewer.id === makeupArtistId && interviewer.role === 'makeup_artist'
      );
    });

    // 2. 筛选已完成的候选人（最近5个）
    // 条件：status > 'interview_scheduled' 且曾经被分配给当前化妆师
  ...
  onEvaluate(e) {
    ...
    console.log('[化妆师工作台] 跳转到评价页面:', candidateId);

    wx.navigateTo({
      url: `/pages/recruit/makeup-artist-evaluation/makeup-artist-evaluation?candidateId=${candidateId}`
    });
  },
